...
`umi_folder_to_lerobot.py` 适用于已经平摊、并且每个 episode 都有 `camera/hand_pose.csv` 的 UMI 任务根目录。默认扫描根目录下全部直接子目录，也可以用 `--episode-list` 传入筛选后的 episode basename 清单。主流程命令见仓库根目录 `README.md`。
...
/home/dzq/openpi/.venv/bin/python \
  ...
  --target /mnt/data/dzq/umi_v2/datasets/task_v2_x2_lerobot_10hz_h50 \
  --repo-id dzq/task_v2_x2_lerobot_10hz_h50 \
...
要改 FPS 或 action horizon，同时修改路径、repo ID，以及 `--fps` / `--action-horizon`。
