...
* the three observation videos are the E6 right eye, cam1, and cam0; this
  task's user-confirmed mapping is cam1 -> left wrist and cam0 -> right wrist.
  This mapping affects videos only, never numeric state/action hand channels.
...
The native files retain acquisition IDs cam0/cam1.  This task's user-confirmed
OpenPI contract maps them to the canonical LeRobot keys ``right_wrist_rgb`` and
``left_wrist_rgb`` respectively; the source acquisition IDs remain recorded
...
VIDEO_KEYS = {
    ...
    "observation.images.left_wrist_rgb": "cam1",
    "observation.images.right_wrist_rgb": "cam0",
}
...
def discover_episodes(
    ...
    # Exempt only the explicitly supplied regular allowlist file. Keep rejecting
    # other visible files; never move/delete the user's selection list.
    allowlist = allowlist_path.resolve(strict=True) if allowlist_path is not None else None
    visible = [path for path in children if not path.name.startswith(".") and not (
        selected_episode_ids is not None and allowlist is not None
        and path == allowlist and path.is_file() and not path.is_symlink()
    )]
    invalid = [
        {
            "name": path.name,
...
def encode_frame_selection(
    ...
    if rotation_degrees not in (0, 180):
        raise ValueError("Only exact 0/180-degree image rotations are supported")
    ...
    run([
        "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-threads", "2", *input_args,
        "-i", str(source),
        "-an", "-filter_threads", "1", "-vf", ",".join(filters), "-c:v", "libx264", "-threads", "2", "-preset", "medium",
    ...
    exact_pixel_proof = None
    if crf == 0:
        def frame_hashes(command: list[str]) -> list[str]:
            from fractions import Fraction
            output = subprocess.check_output(command, text=True, stderr=subprocess.PIPE)
            timebase = next(Fraction(line.split(":", 1)[1].strip())
                            for line in output.splitlines() if line.startswith("#tb 0:"))
            rows = [line.split(",") for line in output.splitlines()
                    if line.strip() and not line.startswith("#")]
            if any(int(parts[2]) * timebase != Fraction(i, output_fps) for i, parts in enumerate(rows)):
                raise RuntimeError(f"Video presentation times do not match {output_fps} Hz: {target}")
            return [parts[-1].strip() for parts in rows]
        reference = frame_hashes([
            "ffmpeg", "-v", "error", "-threads", "2", *input_args, "-i", str(source),
            "-an", "-filter_threads", "1", "-vf", ",".join(filters), "-r", str(output_fps),
            "-vsync", "cfr", "-pix_fmt", "yuv420p", "-threads", "1", "-f", "framemd5", "-",
        ])
        actual = frame_hashes([
            "ffmpeg", "-v", "error", "-threads", "2", "-i", str(target), "-an",
            "-pix_fmt", "yuv420p", "-threads", "1", "-f", "framemd5", "-",
        ])
        if len(actual) != length or actual != reference:
            raise RuntimeError(f"Lossless pixel verification failed: {source} -> {target}")
        exact_pixel_proof = hashlib.sha256(json.dumps(actual).encode()).hexdigest()
    return {
        "frames": length,
        "width": width,
        "height": height,
        ...
        "source_frame_indices_sha256": hashlib.sha256(frame_indices.tobytes()).hexdigest(),
        "crf": crf,
        "decoded_reference_verified": crf == 0,
        "presentation_timestamps_verified": crf == 0,
        "decoded_frame_hashes_sha256": exact_pixel_proof,
    }
...
def collect_plans(
    ...
    episodes, hidden_directories = discover_episodes(source, selected_episode_ids, args.episode_list)
...
def convert(args: argparse.Namespace) -> int:
    ...
    try:
            ...
            for episode_index, plan in enumerate(plans):
                ...
                for key, role in VIDEO_KEYS.items():
                    ...
                    future = pool.submit(
                        ...
                        0 if role == "head" else args.wrist_rotation_degrees,
                    )
        ...
        final_episodes, final_hidden_directories = discover_episodes(
            source, final_selected_episode_ids, args.episode_list
        )
        ...
        contract = {
            ...
            "video_preprocessing": {
                "wrist_rotation_degrees": {
                    "observation.images.left_wrist_rgb": args.wrist_rotation_degrees,
                    "observation.images.right_wrist_rgb": args.wrist_rotation_degrees,
                },
                "wrist_rotation_stage": "after_resize_before_encoding",
                "camera_acquisition_mapping": {"cam0": "right_wrist", "cam1": "left_wrist"},
                "numeric_hand_channels_swapped": False,
                "encoding_crf": args.crf,
                ...
                "cam0_output_key": "observation.images.right_wrist_rgb",
                "cam1_output_key": "observation.images.left_wrist_rgb",
                "cam0_cam1_physical_hand_mapping": (
                    "user-confirmed task_electric mapping cam0=right wrist, cam1=left wrist; "
        ...
        shutil.copy2(Path(__file__).with_name("publish_repaired_dataset.py"),
                     staging / "meta/publish_repaired_dataset.py")
        reserve_and_rename(staging, target)
...
def parse_args() -> argparse.Namespace:
    ...
    convert_parser.add_argument("--wrist-rotation-degrees", type=int, choices=(0, 180), default=0)
    convert_parser.add_argument(
