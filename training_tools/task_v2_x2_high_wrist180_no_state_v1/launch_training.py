#!/usr/bin/env python3
"""Validate a fresh UMI H50 NO-state run and launch when all eight GPUs are idle.
...
def validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected = {"name": args.config, "model_pi05": True, "action_dim": 32,
                "action_horizon": 50, "discrete_state_input": False,
                "state_input_mode": "none", "state_values_used_for_conditioning": False,
                ...
                "batch_size": 64, "num_workers": 32, "num_train_steps": 10000,
    ...
    fingerprint_files = [contract_path, info_path, asset / "norm_stats.json",
                         ...
                         dataset / "meta/head_video_repair.json",
                         dataset / "meta/head_video_publication.json",
                         dataset / "meta/wrist_rotation.json",
    ...
    for name in ("track_pass_episodes.txt", "selected_85_episodes.txt",
                 "excluded_10_episodes.txt", "selection_audit.json", "converter_strict.py",
                 "rotate_wrist_dataset.py", "repair_head_videos.py", "publish_repaired_dataset.py",
