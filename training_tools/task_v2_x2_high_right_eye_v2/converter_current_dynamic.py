...
* every requested-rate E6 observation is emitted.  The first body-delta state
  is identity because no preceding sample exists; action slots beyond the
  episode boundary repeat its terminal target and are explicitly masked by
  ``action_is_pad``;
* camera/hand freshness checks are audit signals rather than crop conditions.
  A wrist camera that starts late is filled only by repeating its nearest real
  first frame, and finger pre-roll uses the nearest real hand-state packet;
* the E6 right eye is the full right half of its actual ffprobe raster;
  stale session dimensions are recorded but never used for cropping;
...
4. builds and preserves the complete requested output-rate grid; timestamp,
   uniqueness, pose-transition, and hand-state freshness checks are recorded
   per row for audit and never crop the episode;
...
class EpisodePlan:
    ...
    sample_alignment_valid: np.ndarray
    source_fps: int
    source_stride: int
    camera_ratio: int
    cam0_offset: int
...
def sample_absolute_hand(
    ...
    if alignment == "causal":
        ...
        # Back-fill the first real packet during capture pre-roll.  ``fresh``
        # remains false and the signed delta remains positive, so the
        # substitution is explicit without dropping the opening observations.
...
def build_future_indices(
    timeline_length: int, action_horizon: int
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if timeline_length < 1 or action_horizon <= 0:
        raise ValueError("timeline_length and action_horizon must be positive")
    anchor_positions = np.arange(timeline_length, dtype=np.int64)
    requested = anchor_positions[:, None] + np.arange(
        1, action_horizon + 1, dtype=np.int64
    )[None, :]
    action_is_pad = requested >= timeline_length
    future_positions = np.minimum(requested, timeline_length - 1)
...
def make_plan(
    ...
    grid_alignment_valid = grid_sample_valid & grid_transition_valid
    runs = valid_sample_sequences(grid_sample_valid, grid_transition_valid)
    # Quality runs remain useful audit information, but they no longer choose
    # which observations exist.  One source directory always emits its full
    # requested-rate E6 grid as one output episode.
    valid_run_index = -1
    run_start, run_end = 0, len(grid_rows)
    ...
    source_quality_common = {
        ...
        "discarded_singleton_sequence_count": 0,
        ...
        "invalid_output_grid_points_retained": int((~grid_alignment_valid).sum()),
        "timeline_policy": "preserve_complete_requested_rate_e6_grid",
        "strict_output_grid_points": int(grid_alignment_valid.sum()),
        "strict_output_grid_retention_ratio": 1.0,
        "strict_cam0_max_abs_delta_ns": int(np.max(np.abs(cam0_delta[grid_rows]))),
        "strict_cam1_max_abs_delta_ns": int(np.max(np.abs(cam1_delta[grid_rows]))),
    ...
    sample_alignment_valid = grid_alignment_valid[anchor_positions]
    ...
    grid_run_membership = np.full(len(grid_rows), -1, dtype=np.int64)
    for record_index, (record_start, record_end) in enumerate(runs):
        grid_run_membership[record_start:record_end] = record_index
    strict_rows: list[dict[str, Any]] = []
    for position, row in enumerate(sample_rows):
        strict_rows.append({
            "source_grid_index": run_start + position,
            "episode_grid_index": position,
            "valid_run_index": int(grid_run_membership[run_start + position]),
            "alignment_valid": int(grid_alignment_valid[run_start + position]),
            "sample_valid": int(grid_sample_valid[run_start + position]),
            "transition_valid": int(grid_transition_valid[run_start + position]),
            "pair_hand_fresh": int(pair_hand_fresh[row]),
    ...
    quality = {
        **source_quality_common,
        ...
        "invalid_grid_points_retained": int((~sample_alignment_valid).sum()),
        "hand_stale_grid_points_retained": int((~pair_hand_fresh[sample_rows]).sum()),
    ...
    return EpisodePlan(
        ...
        sample_alignment_valid=sample_alignment_valid,
...
def right_eye_crop_xywh(width: int, height: int) -> tuple[int, int, int, int]:
    """Select the complete right half of a decoded side-by-side E6 image.

    The source dimensions and the crop's x/y/width/height must be compatible
    with 4:2:0 chroma sampling; do not let ffmpeg silently round an odd crop.
    """
    if any(isinstance(value, bool) or not isinstance(value, int) or value <= 0
           for value in (width, height)):
        raise ValueError(f"E6 dimensions must be positive integers: {width!r}x{height!r}")
    if width % 2 or height % 2:
        raise ValueError(f"E6 decoded dimensions must be even for YUV420: {width}x{height}")
    eye_width = width // 2
    if eye_width % 2:
        raise ValueError(
            f"E6 right-eye x and width must be even for YUV420: "
            f"decoded_width={width}, right_eye_x=right_eye_width={eye_width}"
        )
    return eye_width, 0, eye_width, height


def probe_e6_right_eye(
    source: Path, session_metadata: Path | None = None,
) -> dict[str, Any]:
    """Probe the actual HEVC raster, then derive an audited SBS right-eye crop.

    E6 session metadata can contain obsolete dimensions. It is read for audit,
    never used to position the crop. A bounded larger probe is attempted if the
    small probe cannot resolve the stream; both attempts remain in the audit.
    The capture convention is left eye then right eye. Raster headers cannot
    independently prove physical camera identity, mirroring, or rectification.
    """
    source = Path(source)
    session_metadata = (
        Path(session_metadata) if session_metadata is not None
        else source.parent.parent / "meta" / "e6_stream_session.json"
    )
    attempts: list[dict[str, Any]] = []
    stream: dict[str, Any] | None = None
    for probe_size, analysis_us in ((32768, 0), (1048576, 2000000)):
        attempt: dict[str, Any] = {
            "probe_size_bytes": probe_size, "analyzeduration_us": analysis_us,
        }
        command = [
            "ffprobe", "-v", "error", "-f", "hevc",
            "-probesize", str(probe_size), "-analyzeduration", str(analysis_us),
            "-select_streams", "v:0",
        ...
        try:
            ...
            if result.returncode:
                raise ValueError(f"ffprobe failed: {result.stderr.strip()[:1000]}")
            streams = json.loads(result.stdout).get("streams", [])
            if len(streams) != 1:
                raise ValueError(f"expected one selected video stream, got {len(streams)}")
            candidate = streams[0]
            if candidate.get("codec_name") != "hevc":
                raise ValueError(f"expected HEVC stream, got {candidate.get('codec_name')!r}")
            if any(
                ...
                raise ValueError(f"probe did not resolve positive raster dimensions: {candidate}")
            stream = candidate
            attempt["status"] = "ok"
    ...
    if stream is None:
        raise ValueError(
            f"E6 right-eye probe failed; no crop or encoding is allowed: {source}; "
            f"probe_audit={json.dumps(attempts, ensure_ascii=False)}"
        )
    width, height = stream["width"], stream["height"]
    try:
        x, y, crop_width, crop_height = right_eye_crop_xywh(width, height)
    except ValueError as error:
        raise ValueError(
            f"E6 right-eye crop rejected: {source}; {error}; "
            f"probe_audit={json.dumps(attempts, ensure_ascii=False)}"
        ) from error
    ...
    metadata_error = None
    try:
        metadata = json.loads(session_metadata.read_text(encoding="utf-8"))
        video = metadata["video"]
        declared_width, declared_height = video["width"], video["height"]
        if any(
            isinstance(value, bool) or not isinstance(value, int) or value <= 0
            for value in (declared_width, declared_height)
        ):
            raise ValueError("metadata video width/height are not positive integers")
        metadata_status = "ok"
    except FileNotFoundError:
        metadata_error = "session metadata file is absent; using probed raster"
    ...
    dimensions_match = (
        ...
        if metadata_status == "ok" else None
    )
    warnings = []
    if len(attempts) > 1:
        warnings.append("Small probe was insufficient; bounded larger probe resolved the raster.")
    if dimensions_match is False:
        warnings.append(
            f"Source metadata declares {declared_width}x{declared_height}, "
            f"but HEVC raster is {width}x{height}; crop uses the HEVC raster."
        )
    if metadata_error:
        warnings.append(metadata_error)
    return {
        "schema": "umi-e6-right-eye-preprocessing",
        "schema_version": 1,
        "source": str(source),
        "actual_width": width,
        "actual_height": height,
        "actual_dimensions_source": "ffprobe HEVC stream raster",
        "codec_name": stream["codec_name"],
        "pix_fmt": stream.get("pix_fmt"),
        "metadata_path": str(session_metadata),
        "declared_width": declared_width,
        "declared_height": declared_height,
        "metadata_status": metadata_status,
        "metadata_dimensions_match": dimensions_match,
        "layout": "side_by_side",
        "eye_order_assumption": "left_then_right_capture_convention",
        "selected_eye": "right",
        "selected_region": "complete_right_half_of_decoded_raster",
        "crop_xywh": [x, y, crop_width, crop_height],
        "source_crop": f"crop={crop_width}:{crop_height}:{x}:{y}",
...
def encode_frame_selection(
    ...
    if (
        ...
        raise ValueError(f"video frame selection must be non-empty and non-decreasing: {source}")
    length = len(frame_indices)
    prefix_repeats = 0
    while (
        prefix_repeats + 1 < length
        and frame_indices[prefix_repeats + 1] == frame_indices[0]
    ):
        prefix_repeats += 1
    suffix_repeats = 0
    while (
        suffix_repeats + 1 < length - prefix_repeats
        and frame_indices[length - suffix_repeats - 2] == frame_indices[-1]
    ):
        suffix_repeats += 1
    core_indices = frame_indices[prefix_repeats:length - suffix_repeats]
    if np.any(np.diff(core_indices) <= 0):
        raise ValueError(
            f"video frame reuse is supported only at a source boundary: {source}"
        )
    target.parent.mkdir(parents=True, exist_ok=True)
    select_expression = "+".join(f"eq(n\\,{int(index)})" for index in core_indices)
    filters = [f"select='{select_expression}'"]
    if source_crop is not None:
        filters.append(source_crop)
    filters.append(f"setpts=N/({output_fps}*TB)")
    if prefix_repeats or suffix_repeats:
        filters.append(
            "tpad="
            f"start={prefix_repeats}:start_mode=clone:"
            f"stop={suffix_repeats}:stop_mode=clone"
        )
        # tpad may prepend negative timestamps. Rebase once more so CFR output
        # contains exactly one packet per requested observation.
        filters.append(f"setpts=N/({output_fps}*TB)")
    filters.append(resize_filter(width, height, resize_mode))
    ...
    return {
        ...
        "source_boundary_repeats": {
            "prefix": prefix_repeats,
            "suffix": suffix_repeats,
        },
...
def build_features(width: int, height: int, action_horizon: int) -> dict[str, dict[str, Any]]:
    features: dict[str, dict[str, Any]] = {
        ...
        "sample_alignment_valid": {
            "dtype": "int64", "shape": (1,),
            "names": ["camera_pose_and_hand_quality_within_limits"],
        },
...
def write_parquet(
    ...
    arrays = {
        ...
        "sample_alignment_valid": plan.sample_alignment_valid.astype(np.int64)[:, None],
...
def collect_plans(
    ...
    for index, episode in enumerate(episodes):
        try:
            ...
            plan.quality["head_video_preprocessing"] = probe_e6_right_eye(
                plan.files.head_video
            )
            plans.append(plan)
...
def inspect(args: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "head_video_metadata_warnings": [
            {"source_episode_id": plan.output_episode_id,
             **plan.quality["head_video_preprocessing"]}
            for plan in plans
            if plan.quality["head_video_preprocessing"]["warnings"]
        ],
    ...
    try:
            ...
            for episode_index, plan in enumerate(plans):
                sources = {
                    "head": (
                        ...
                        plan.quality["head_video_preprocessing"]["source_crop"], "hevc",
                    ),
                    "cam0": (
        ...
        contract = {
            ...
            "camera_alignment": {
                ...
                "camera_policy": (
                    f"complete E6 {args.fps}Hz grid, nearest timestamp; "
                    "boundary camera gaps repeat the nearest real edge frame and all "
                    "quality failures remain explicit audit flags"
                ),
            ...
            "video_preprocessing": {
                "head_rgb_source": "camera/e6_rgb.h265 (E6 side-by-side stereo; probed raster)",
                "head_rgb_crop": "complete right half: x=decoded_width/2, y=0, width=decoded_width/2, height=decoded_height",
                "head_rgb_dimension_source": "ffprobe HEVC stream raster, not session metadata",
                "head_rgb_metadata_mismatch_count": sum(
                    plan.quality["head_video_preprocessing"]["metadata_dimensions_match"] is False
                    for plan in plans
                ),
                "head_rgb_episode_audit_key": "head_video_preprocessing",
            ...
            "training_contract": {
                ...
                "alignment_quality_audit_key": "sample_alignment_valid",
        ...
        (staging / "README.md").write_text(
            ...
            "Each source directory maps to exactly one complete LeRobot episode; "
            "alignment/freshness failures are retained and exposed through "
            "sample_alignment_valid rather than cropping the timeline. "
...
def self_test() -> int:
    ...
    for output_fps, action_horizon in ((10, 50), (30, 90)):
        ...
        if (
            anchors.shape != (4,)
            or future.shape != (4, action_horizon)
            or pad.shape != (4, action_horizon)
            or not pad[-1].all()
            or pad[0, 0]
            or not pad[0, 3:].all()
            or int(future.max()) != 3
    ...
    print(
        ...
        "quality-run audit, duplicate packet conflict, hand pose, SE(3), padding"
    )
    return 0
