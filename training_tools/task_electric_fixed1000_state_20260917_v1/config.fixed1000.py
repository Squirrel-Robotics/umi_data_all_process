...
# Keep historical experiments immutable. The new run changes only the hand
# normalization asset, identity, and documentation; model/optimizer stay equal.
_electric_fixed1000_base = next(
    config for config in _CONFIGS
    if config.name == "pi05_umi_task_electric_10hz_h50_wrist180_state_20k_b64_w128_8gpu_v1"
)
_CONFIGS.append(dataclasses.replace(
    _electric_fixed1000_base,
    name="pi05_umi_task_electric_10hz_h50_wrist180_state_fixed1000_20k_b64_w128_8gpu_v1",
    data=dataclasses.replace(
        _electric_fixed1000_base.data,
        assets=dataclasses.replace(
            _electric_fixed1000_base.data.assets,
            asset_id="umi_task_electric_10hz_h50_right_eye_wrist180_fixed1000_v1",
        ),
    ),
    policy_metadata={
        **_electric_fixed1000_base.policy_metadata,
        "hand_normalization": {
            "kind": "fixed_linear_bounds", "raw_count_bounds": [0, 1000],
            "raw_units": "0.1 degree/count", "dataset_degree_bounds": [0, 100],
            "dimensions": list(range(18, 30)), "applies_to": ["state", "actions"],
            "eef_normalization": "unchanged_empirical_quantiles",
            "inference_hand_units": "absolute_degrees",
        },
    },
))
