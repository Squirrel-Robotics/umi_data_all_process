...
def main():
    ...
    for key,value in {'batch_size':64,'num_workers':128,'num_train_steps':20000,'save_interval':1000,
    ...
    baseline=normalize.deserialize_json((BASE_ASSET/'norm_stats.json').read_text())
    require(audit['finger_scaling']['raw_count_bounds']==[0,1000]
            and audit['finger_scaling']['dataset_degree_bounds']==[0,100]
            and audit['finger_scaling']['dimensions']==list(range(18,30)),'Wrong fixed hand bounds')
    require(audit['base_generator_sha256']==digest(BASE_NORM_TOOL)
            and audit['parent_norm_stats_sha256']==digest(BASE_ASSET/'norm_stats.json'),'Normalization baseline changed')
    for key in ('state','actions'):
        for field in ('mean','std','q01','q99'):
            np.testing.assert_allclose(getattr(saved[key],field),getattr(recomputed[key],field),rtol=1e-12,atol=1e-12)
            np.testing.assert_array_equal(getattr(saved[key],field)[:18],getattr(baseline[key],field)[:18])
        np.testing.assert_array_equal(saved[key].q01[18:30],0)
        np.testing.assert_array_equal(saved[key].q99[18:30],100)
        for field in ('mean','std'):
            np.testing.assert_array_equal(getattr(saved[key],field),getattr(baseline[key],field))
    normalizer=transforms.Normalize(saved,use_quantiles=True)
    unnormalizer=transforms.Unnormalize(saved,use_quantiles=True)
    ...
    for plan,manifest in zip(plans,contract['episodes']):
        ...
        original={'state':np.asarray(plan.state,dtype=np.float64),'actions':np.asarray(plan.action,dtype=np.float64)}
        scaled=normalizer(copy.deepcopy(original))
        restored=unnormalizer(copy.deepcopy(scaled))
        for key in ('state','actions'):
            require(np.isfinite(scaled[key]).all(),'Non-finite normalized targets')
            require(np.all((original[key][...,18:30]>=0)&(original[key][...,18:30]<=100)), 'Hand data outside 0..100 degrees')
            require(np.all((scaled[key][...,18:30]>=-1.000001)&(scaled[key][...,18:30]<=1.000001)), 'Normalized fingers outside fixed bounds')
            np.testing.assert_allclose(restored[key],original[key],rtol=0,atol=1e-10)
    ...
    require(data.use_quantile_norm,'Fixed q01/q99 bounds require the quantile transform')
    ...
    report={'status':'complete','config':CONFIG,'episodes':115,'frames':24182,'verified_videos':checked_videos,
            'finger_scaling':audit['finger_scaling'],'all_episodes_normalization_roundtrip_verified':True,
